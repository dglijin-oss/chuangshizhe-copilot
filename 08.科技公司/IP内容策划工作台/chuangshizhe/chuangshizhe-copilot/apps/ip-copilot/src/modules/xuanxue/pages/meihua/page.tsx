...
export default function MeihuaPage() {
  ...
  const [form, setForm] = useState({
    method: 'time' as 'time' | 'number' | 'character',
    numbers: '',
    question: '',
  })
  ...
  const canSubmit = form.method === 'time' || (form.numbers && form.numbers.length >= 2)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch('/api/xuanxue/meihua', {
        method: 'POST',
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0A1628] via-[#0D1F3C] to-[#112240] text-white">
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 text-sm text-violet-300 bg-violet-500/10 rounded-full px-4 py-1.5 mb-4">
            <span>🌸</span> 梅花易数
          </div>
          <h1 className="text-2xl md:text-3xl font-bold mb-2">以数起卦 · 时间起卦</h1>
          <p className="text-gray-400 text-sm">简洁起卦法，快速断卦，适合日常占卜</p>
        ...
        {!result && (
          <form onSubmit={handleSubmit} className="bg-white/5 border border-white/10 rounded-xl p-6">
            <div className="mb-6">
              <label className="block text-xs font-medium text-gray-400 mb-1.5">起卦方式</label>
                ...
                {[
                  ...
                  <button key={opt.value} type="button"
                    ...
                    className={`flex-1 py-2.5 rounded-lg text-sm font-medium transition-all ${
                      form.method === opt.value ? 'bg-violet-600 text-white' : 'bg-white/10 text-gray-400 hover:bg-white/20'
                    }`}
                  >{opt.label}</button>
                ))}
              </div>
            ...
            {form.method === 'number' && (
              <div className="mb-6">
                <label className="block text-xs font-medium text-gray-400 mb-1.5">输入两个数字</label>
                <input type="text" value={form.numbers} onChange={(e) => setForm({ ...form, numbers: e.target.value })}
                  placeholder="如：3 8 或 12 7"
                  className="w-full bg-white/10 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-violet-500 transition-colors"
                />
              </div>
            )}
            ...
            <button type="submit" disabled={!canSubmit || loading}
              className={`w-full py-3 rounded-lg text-sm font-medium transition-all ${
                canSubmit && !loading ? 'bg-violet-600 hover:bg-violet-500 text-white' : 'bg-white/10 text-gray-500 cursor-not-allowed'
              }`}>
          ...
          <div className="space-y-6">
            <div className="bg-white/5 border border-white/10 rounded-xl p-5">
              <h3 className="text-sm font-bold text-violet-300 mb-4 flex items-center gap-2">
                <span className="w-1 h-4 bg-violet-500 rounded-full" />占卜结果
              </h3>
            ...
            <div className="flex justify-center gap-3 pt-4">
              <button onClick={() => { setResult(null); setPointsInfo(null) }}
                className="bg-violet-600 hover:bg-violet-500 text-white px-8 py-2.5 rounded-full text-sm font-medium transition-colors">
                重新起卦
              </button>
            </div>
          </div>
