...
export default function ZiweiPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch('/api/xuanxue/ziwei', {
        method: 'POST',
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0A1628] via-[#0D1F3C] to-[#112240] text-white">
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 text-sm text-indigo-300 bg-indigo-500/10 rounded-full px-4 py-1.5 mb-4">
            <span>⭐</span> 紫微斗数
          </div>
          <h1 className="text-2xl md:text-3xl font-bold mb-2">紫微斗数排盘</h1>
          <p className="text-gray-400 text-sm">
            以出生时间排布十二宫位，四化飞星，命盘详解
          </p>
        ...
        {!result && (
          <form onSubmit={handleSubmit} className="bg-white/5 border border-white/10 rounded-xl p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <input type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })}
                  className="w-full bg-white/10 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-indigo-500 transition-colors" required />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-400 mb-1.5">
                ...
                <select value={form.hour} onChange={(e) => setForm({ ...form, hour: e.target.value })}
                  className="w-full bg-white/10 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white focus:outline-none focus:border-indigo-500 transition-colors appearance-none" required>
            ...
            <div className="mb-6">
              <label className="block text-xs font-medium text-gray-400 mb-1.5">性别 <span className="text-red-400">*</span></label>
              <div className="flex gap-3">
                {[{ value: '男', label: '男 (乾造)' }, { value: '女', label: '女 (坤造)' }].map((opt) => (
                  <button key={opt.value} type="button"
                    onClick={() => setForm({ ...form, gender: opt.value as '男' | '女' })}
                    className={`flex-1 py-2.5 rounded-lg text-sm font-medium transition-all ${
                      form.gender === opt.value ? 'bg-indigo-600 text-white' : 'bg-white/10 text-gray-400 hover:bg-white/20'
                    }`}>{opt.label}</button>
                ))}
              </div>
            </div>
            ...
            <div className="mb-6">
              <label className="block text-xs font-medium text-gray-400 mb-1.5">想重点了解（可选）</label>
              <input type="text" value={form.question} onChange={(e) => setForm({ ...form, question: e.target.value })}
                placeholder="如：事业运、感情运、财运"
                className="w-full bg-white/10 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-indigo-500 transition-colors" />
            </div>

            <button type="submit" disabled={!canSubmit || loading}
              className={`w-full py-3 rounded-lg text-sm font-medium transition-all ${
                canSubmit && !loading ? 'bg-indigo-600 hover:bg-indigo-500 text-white' : 'bg-white/10 text-gray-500 cursor-not-allowed'
              }`}>
          ...
          <div className="space-y-6">
            <div className="bg-white/5 border border-white/10 rounded-xl p-5">
              <h3 className="text-sm font-bold text-indigo-300 mb-4 flex items-center gap-2">
                <span className="w-1 h-4 bg-indigo-500 rounded-full" />紫微斗数命盘
              </h3>
            ...
            <div className="flex justify-center gap-3 pt-4">
              <button onClick={() => { setResult(null); setPointsInfo(null) }}
                className="bg-indigo-600 hover:bg-indigo-500 text-white px-8 py-2.5 rounded-full text-sm font-medium transition-colors">
                重新排盘
              </button>
            </div>
          </div>
