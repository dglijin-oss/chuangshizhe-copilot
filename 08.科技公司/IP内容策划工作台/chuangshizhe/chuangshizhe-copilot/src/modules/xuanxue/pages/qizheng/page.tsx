...
export default function QizhengPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch('/api/xuanxue/qizheng', {
        method: 'POST',
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0A1628] via-[#0D1F3C] to-[#112240] text-white">
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 text-sm text-emerald-300 bg-emerald-500/10 rounded-full px-4 py-1.5 mb-4">
            <span>🪐</span> 七政四余
          </div>
          <h1 className="text-2xl md:text-3xl font-bold mb-2">七政四余排盘</h1>
          <p className="text-gray-400 text-sm">
            星曜落宫，宫位分析，古代天文星象命理之学
          </p>
          ...
          <form onSubmit={handleSubmit} className="bg-white/5 border border-white/10 rounded-xl p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <input type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })}
                  className="w-full bg-white/10 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-emerald-500 transition-colors" required />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-400 mb-1.5">
                ...
                <select value={form.hour} onChange={(e) => setForm({ ...form, hour: e.target.value })}
                  className="w-full bg-white/10 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white focus:outline-none focus:border-emerald-500 transition-colors appearance-none" required>
            ...
            <div className="mb-6">
              ...
              <input type="text" value={form.question} onChange={(e) => setForm({ ...form, question: e.target.value })}
                ...
                className="w-full bg-white/10 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-emerald-500 transition-colors" />
            </div>

            <button type="submit" disabled={!canSubmit || loading}
              className={`w-full py-3 rounded-lg text-sm font-medium transition-all ${
                canSubmit && !loading ? 'bg-emerald-600 hover:bg-emerald-500 text-white' : 'bg-white/10 text-gray-500 cursor-not-allowed'
              }`}>
          ...
          <div className="space-y-6">
            <div className="bg-white/5 border border-white/10 rounded-xl p-5">
              <h3 className="text-sm font-bold text-emerald-300 mb-4 flex items-center gap-2">
                <span className="w-1 h-4 bg-emerald-500 rounded-full" />排盘结果
              </h3>
            ...
            <div className="flex justify-center gap-3 pt-4">
              <button onClick={() => { setResult(null); setPointsInfo(null) }}
                className="bg-emerald-600 hover:bg-emerald-500 text-white px-8 py-2.5 rounded-full text-sm font-medium transition-colors">
                重新排盘
              </button>
            </div>
          </div>
